...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option
				...
				.setDescription(
					strFormat(LANG.common.optionDescription.graphLabel, [
						LANG.common.defaultValues.graphLabel,
					]),
				)
				.setRequired(false),
		)
		.addStringOption((option) =>
			option
				...
				.setDescription(
					strFormat(LANG.common.optionDescription.graphTitle, [
						strFormat(LANG.commands.graph.defaultTitle, [
							LANG.common.optionDescription.userPlaceholder,
						]),
					]),
				)
				.setRequired(false),
	...
	execute: async function (interaction) {
		...
		if (
			...
			linergb = interaction.options.getString(
				LANG.commands.graph.options.lineColor.name,
			);
		}
		...
		if (
			...
			bgtheme = interaction.options.getString(
				LANG.commands.graph.options.backgroundTheme.name,
			);
		}
		...
		if (interaction.options.getString(LANG.commands.graph.options.title.name)) {
			graphtitle = interaction.options.getString(
				LANG.commands.graph.options.title.name,
			);
		}
		...
		if (interaction.options.getString(LANG.commands.graph.options.label.name)) {
			label = interaction.options.getString(
				LANG.commands.graph.options.label.name,
			);
		}
		...
		if (
			interaction.options.getBoolean(
				LANG.commands.graph.options.beginAtZero.name,
			)
		) {
			AtZero = interaction.options.getBoolean(
				LANG.commands.graph.options.beginAtZero.name,
			);
		}

		const valuesString = interaction.options.getString(
