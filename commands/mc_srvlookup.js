	...
	execute: async function (interaction) {
		...
		try {
			dns.resolveSrv(domainName, (err, records) => {
				if (err) {
					console.error(
						strFormat(LANG.commands.mcSrvlookup.resolutionError, {
							domain,
							message: err.message,
						}),
					);
				...
				if (records.length === 0) {
					console.log(
						strFormat(LANG.commands.mcSrvlookup.notFoundError, [domain]),
					);
					interaction.reply(
						strFormat(LANG.commands.mcSrvlookup.notFoundError, [domain]),
					);
			...
			console.error(
				strFormat(LANG.commands.mcSrvlookup.generalError, [error.message]),
			);
