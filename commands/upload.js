...
module.exports = {
    ...
    execute: async function (
        ...
        const file = interaction.options.getAttachment(LANG.commands.upload.options.file.name);
        ...
        try {
            ...
            let filename = interaction.options.get(LANG.commands.upload.options.filename.name)?.value;
            let isPrivate = interaction.options.get(LANG.commands.upload.options.private.name)?.value == true;
            ...
            dmChannel.send({
                embeds: [{
                    title: strFormat(LANG.commands.upload.result.title, {
                        ...
                        isPrivate: isPrivate ? LANG.commands.upload.result.isPrivate.yes : LANG.commands.upload.result.isPrivate.no
                    }),
                    color: 0x5865f2,
                    fields: [{
            ...
            if (e?.name == "AxiosError" && e?.response?.status) {
                await interaction.editReply({
                    embeds: [{
                        ...
                        description: strFormat(LANG.commands.upload.errorResult.description, {
