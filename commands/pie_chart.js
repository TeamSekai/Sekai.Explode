...
module.exports = {
			...
			option
				...
				.setDescription(
					strFormat(LANG.common.optionDescription.graphTitle, [
						strFormat(LANG.commands.pieChart.defaultTitle, [
	...
	execute: async function (interaction) {
		...
		if (
			interaction.options.getString(LANG.commands.pieChart.options.title.name)
		) {
			graphtitle = interaction.options.getString(
				LANG.commands.pieChart.options.title.name,
			);
		}

		let label = LANG.common.defaultValues.graphLabel;
		if (
			interaction.options.getString(LANG.commands.pieChart.options.label.name)
		) {
			label = interaction.options.getString(
				LANG.commands.pieChart.options.label.name,
			);
		}
