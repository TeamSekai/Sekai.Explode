...
function zeroPadding(num, digits = 2) {
    ...
    const originalString = integerPart == num ? integerPartString : num.toString();
...
class Timespan {
    ...
    compareTo(val) {
        ...
        if (thisMillis < valMillis)
            return -1;
        else if (thisMillis == valMillis)
            return 0;
        else
            return 1;
    }
    ...
    static toString(values) {
        ...
        const belowMinuteString = zeroPadding(secondPart) + (milliPart != 0 ? '.' + zeroPadding(milliPart, 3) : '');

        if (hours == 0)
            return minutePart + ':' + belowMinuteString;
        ...
        if (days == 0)
            return hourPart + ':' + belowHourString;
